/**
 * System prompt da PetVia AI — atendimento WhatsApp para clínicas veterinárias.
 * Usar em Edge Functions / backend (não expor chaves no cliente).
 * Mantenha em sincronia com: supabase/functions/_shared/petviaWhatsappAssistantPrompt.ts
 */
...
/** Monta o system prompt com dados da clínica. */
...
/** Bloco opcional anexado ao system ou como mensagem de contexto. */
...
/** A partir do tipo Clinic do app. */
export function buildPetviaWhatsappSystemPromptFromClinic(clinic: {
  name: string
  city?: string | null
  phone?: string | null
  ...
  return buildPetviaWhatsappSystemPrompt({
    clinic_name: clinic.name,
    city: clinic.city,
    phone: clinic.phone ?? clinic.whatsapp_number,
    working_hours: clinic.opening_hours,
  })
}
